...
### **`Unreleased`**
- **AI Model Upgrade**: Migrated the single-image flow (`/api/v1/generate-from-single`) to `fal-ai/trellis-2`, which takes an `image_url` string input and returns a `.glb` model URL. The multi-image flow (`/api/v1/generate-from-multiple`) remains on `fal-ai/trellis/multi` with the `image_urls` array input.
